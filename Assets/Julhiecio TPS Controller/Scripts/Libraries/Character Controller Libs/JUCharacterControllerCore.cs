...
namespace JUTPS.CharacterBrain
{
    ...
    public class JUCharacterBrain : MonoBehaviour
    {
        ...
        public float SprintingSpeedMax = 2f;
        ...
        public bool SprintOnRunButton = true;
        public bool UnlimitedSprintDuration = true;
        ...
        public struct JUCharacterEvents
        {
            ...
            {
                //Run Event
                if (juchar.IsRunning && calledRun == false) { OnRun.Invoke(); } 
                else if (calledRun == true) { calledRun = false; }

                //Sprint Event
                if (juchar.IsSprinting && calledOnSprinting == false) { OnSprinting.Invoke(); } 
                else if (calledOnSprinting == true) { calledOnSprinting = false; }
                ...
                if (juchar.IsMoving == false && calledStopMoving == false) { OnIdle.Invoke(); } 
                else if (calledStopMoving == true) { calledStopMoving = false; }

                //Fire Mode Event
                if (juchar.FiringMode && calledOnFireMode == false) { OnEnterFireMode.Invoke(); } 
                else if (calledOnFireMode == true) { calledOnFireMode = false; }

                //Exit Fire Mode Event
                if (juchar.FiringMode == false && calledOnExitFireMode == false) { OnExitFireMode.Invoke(); } 
                else if (calledOnExitFireMode == true) { calledOnExitFireMode = false; }
            }
        ...
        protected Transform CreateEmptyTransform(string name = "New Transform", 
        ...
        public virtual void MoveForward(float SpeedMultiplier)
        {
            if (SetRigidbodyVelocity)
            {
                ...
                rb.velocity = transform.forward * SpeedMultiplier * Speed + transform.up * localVelocity.y; // 정규화 안하는게 더 게임답긴 하다 
                // rb.velocity = rb.velocity; 왜 중복으로 되어있지 
        ...
        public void InAirMovementControl(bool JumpInert = true)
        {
            ...
            if (IsGrounded)
            {
                if (JumpInert) // Inert = not moving or not able to move 
                {
                    LastX = HorizontalX;
                    LastY = VerticalY;
        ...
        protected virtual void DoFreeMovement(bool FiringMode)
        {
            ...
            if (IsGrounded && CanMove && !RootMotion)
            {
                ...
                else
                {
                    if (SetRigidbodyVelocity)
                    {
                        var localVelocity = DirectionTransform.InverseTransformDirection(rb.velocity);
                        rb.velocity = DirectionTransform.forward * VelocityMultiplier * Speed + transform.up * localVelocity.y;
                    }
                    else
                    {
                        transform.Translate(DirectionTransform.forward * VelocityMultiplier * Speed * Time.deltaTime, Space.World);
                    }
                }
            }
            ...
            if (IsMoving && IsMeleeAttacking == false) 
            {
                // Crouch State
                ...
                // Sprint State
                if (IsGrounded == true && IsRunning == true && IsSprinting == true && IsCrouched == false && IsProne == false && WallAhead == false)
                {
                    VelocityMultiplier = Mathf.Lerp(VelocityMultiplier, SprintingSpeedMax - GroundAngleDesacelerationValue(), 6 * Time.deltaTime);
                }
            ...
            if (IsRunning && IsSprinting)
            {
                ...
                {
                    // CurrentSprintSpeedIntensity = Mathf.Lerp(CurrentSprintSpeedIntensity, SprintingSpeedMax, SprintingAcceleration * Time.deltaTime);
                    VelocityMultiplier = Mathf.Lerp(VelocityMultiplier, SprintingSpeedMax - GroundAngleDesacelerationValue(), 4.5f * Time.deltaTime);
                }

                //Speed Down
                if (ReachedMaxSprintSpeed == true)
                {
                    VelocityMultiplier -= SprintingDeceleration * Time.deltaTime;
        ...
        protected virtual void UseRightHandItem(bool ShotInput, bool ShotDownInput)
        {
            ...
            if (!IsWeaponEquiped || IsRolling) return;
        ...
        protected virtual void UseLeftHandItem(bool ShotInput, bool ShotDownInput)
        {
            ...
            if (!FiringMode || !IsWeaponEquiped || IsRolling) return;
        ...
        public virtual void _Jump()
        {
            ...
            if (IsGrounded == false || IsRolling == true || IsDriving == true || CanJump == false || IsProne || IsRagdolled)
            {
                _GetUp();
        ...
        public virtual void _DoPunch()
        {
            ...
            if (AnimatorParameters.Punch != "")
            {
                ...
                if(anim.GetCurrentAnimatorStateInfo(0).IsName("Punch Attack 1")||anim.GetCurrentAnimatorStateInfo(0).IsName("Punch Attack 2"))
                {IsPunching = true;}
                else {IsPunching = false;}
            }
        }

        public virtual void DefaultUseOfAllItems(bool ShotInput, 
                                                bool ShotInputDown = false, 
                                                bool ReloadInput = false, 
                                                bool AimInput = false, 
                                                bool AimInputDown = false, 
                                                bool MeleeAttackInput = false)
        {
            if (HoldableItemInUseLeftHand != null || HoldableItemInUseRightHand != null)
            {
                ...
                if (WeaponInUseRightHand != null)
                {
                    if (WeaponInUseRightHand.AimMode == Weapon.WeaponAimMode.None) 
                    { IsAiming = false; AimInput = false; AimInputDown = false; }
                }
        ...
        public void SwitchToItem(int id = -1, bool RightHand = true)
        {
            ...
            IsWeaponEquiped = Inventory.IsItemSelected;
    ...
    public class JUAnimatorParameters
    {
        ...
        public string IsPunching = "IsPunching";
