...
{
    /*  @붙이는 이유 
        유니티에서 인스턴스 변수 이름 앞에 @를 붙이는 것은 C#의 기능인데, 주로 예약어와 동일한 이름을 사용할 때 충돌을 피하기 위해 사용됩니다. 예를 들어, class, event, namespace 같은 C#의 예약어를 변수 이름으로 사용할 수 없지만, @class, @event, @namespace처럼 앞에 @를 붙이면 사용할 수 있습니다.

        @가 붙어있는 변수는 실제로 @를 제외한 이름과 동일하게 인식됩니다. 즉, @myVariable은 myVariable과 같은 이름으로 인식되지만, @를 붙임으로써 예약어와 충돌을 피할 수 있는 것입니다. 

        클래스 이름 앞에 @를 붙이는 것은 예약어와의 충돌을 피하기 위해서 사용되긴 하지만, 위 코드에서는 굳이 필요하지 않은 경우일 수 있습니다. JUTPSInputControlls는 C#의 예약어가 아니기 때문에 @를 붙이지 않아도 동일하게 동작할 것입니다.

        @를 클래스 이름에 붙였을 때 특별한 의미나 이점은 없고, 오히려 코드 스타일 면에서는 권장되지 않습니다. 보통 @는 변수명에 예약어와 충돌이 있을 때만 사용하는 것이 일반적이므로, 이 경우는 단순히 실수로 붙였거나 불필요하게 사용된 것으로 보입니다.

        다시 말해, @JUTPSInputControlls는 JUTPSInputControlls와 동일하게 동작하며, 특별히 예약어와 충돌하지 않는다면 @는 빼도 문제가 없습니다.
    */
    ...
    public class @JUTPSInputControlls : IInputActionCollection, IDisposable
    {
        ...
        /*  생성자 public @JUTPSInputControlls()
            public @JUTPSInputControlls() 는 프로퍼티(Property)가 아니라, 생성자(Constructor) 입니다. 
            생성자는 클래스의 인스턴스가 생성될 때 자동으로 호출되는 특별한 메서드입니다. 
            클래스의 필드나 프로퍼티에 초기 값을 설정하거나 필요한 초기 설정 작업을 수행합니다. --> InputActionAsset 초기화
            반환 타입이 없으며, 클래스 이름과 동일한 이름을 가집니다.

            프로퍼티는 클래스 내에서 데이터를 캡슐화하고 접근을 제어하는 멤버입니다.
            클래스의 필드에 대한 접근을 제공하며, get과 set 접근자를 통해 값을 읽고 쓸 수 있습니다.
            반환 타입과 이름을 가지며, 일반적으로 public으로 선언되어 외부에서 접근할 수 있습니다.
        */
        public @JUTPSInputControlls()
        {
            /*  verbatim string
                InputActionAsset.FromJson(@"{ ... }")에서 사용된 @는 문자열 리터럴을 나타내는 C#의 "verbatim string" 기능입니다. 
                일반적으로 문자열을 작성할 때, C#에서는 백슬래시(\)를 이스케이프 문자로 사용합니다. 
                예를 들어, 줄바꿈을 의미하는 \n, 탭을 의미하는 \t, 또는 파일 경로를 나타낼 때 C:\\Program Files\\...처럼 사용합니다. 
                하지만 **@**를 붙이면 이스케이프 문자를 무시하고 문자열을 있는 그대로 처리합니다. 
            */
        ...
        /*  readonly 키워드 및 선언과 초기화의 분리 
            readonly 키워드는 C#에서 사용되며, 초기화가 한 번만 가능하다는 의미입니다. readonly 필드는 선언과 동시에 초기화하거나, 생성자에서 한 번만 초기화할 수 있습니다. 그 이후에는 해당 클래스 내에서 다시 값을 변경할 수 없습니다.

            다시 말해, readonly로 선언된 변수는 초기화 이후 수정할 수 없고, 상수와 유사하지만 초기화 시점을 코드 내에서 좀 더 유연하게 설정할 수 있다는 차이점이 있습니다. 초기화는 선언 시 또는 생성자 내에서만 가능합니다.

            1. 필드 선언이 먼저입니다:
            private readonly InputActionMap m_Player;
            이 부분은 클래스의 멤버 변수로, 클래스 정의 시 가장 먼저 선언됩니다.
            
            2. 그 다음 생성자가 호출됩니다:
            m_Player_Move = m_Player.FindAction("Move", throwIfNotFound: true);는 이 멤버 변수를 초기화하는 부분입니다. 
            FindAction 메서드를 통해 "Move"라는 이름의 액션을 찾아 m_Player_Move에 할당합니다.        

            따라서, m_Player_Move는 클래스 내에서 "Move" 액션을 참조하는 데 사용되며, 생성자에서 한 번 초기화된 후에는 변경되지 않습니다.    
        */

        public struct PlayerActions
        {
            #region 위에서 만든 클래스 멤버변수를 참조하는 구조체 형성(일종의 바로가기)
            /*  m_ 을 붙이는 이유 
                m_는 멤버 변수를 나타내는 접두사로, 클래스 내에서 선언된 프라이빗(private) 변수임을 명시적으로 표시합니다.
                Wrapper는 특정 기능이나 클래스를 포장(wrapping) 하는 역할을 하는 객체를 의미합니다. 
                Unity의 새로운 Input System에서 생성된 자동 코드에서는 m_Wrapper라는 변수명이 기본적으로 사용됩니다.
                m_Wrapper는 입력 액션 콜백 인터페이스(IPlayerActions, IUIActions 등)를 관리하고, 액션 이벤트를 콜백 메서드에 연결하는 역할을 합니다.
                m_Wrapper라는 변수명은 코드의 가독성과 유지보수성을 높이기 위해 사용된 명명 규칙 및 관례의 일환입니다. 
                특히, Unity의 새로운 Input System에서 자동 생성된 코드에서는 이러한 변수명이 일관되게 사용되어, 개발자들이 입력 액션과 관련된 로직을 쉽게 이해하고 관리할 수 있도록 돕습니다.
            */
            private @JUTPSInputControlls m_Wrapper;

            /*  PlayerActions 구조체의 생성자(Constructor)
                이 생성자는 PlayerActions 구조체의 인스턴스를 초기화할 때 호출되며, 외부에서 전달된 JUTPSInputControlls 클래스의 인스턴스를 내부(멤버) 변수 m_Wrapper에 할당합니다.
            */          
            public PlayerActions(@JUTPSInputControlls wrapper) { m_Wrapper = wrapper; }
            
            /*  표현식 몸체 멤버(Expression-bodied Members)
                public InputAction @Move => m_Wrapper.m_Player_Move; 는 아래와 같은 의미다 

                private InputAction m_Player_Move;
                public InputAction @Move
                {
                    get { return m_Player_Move; }
                }
                @Move 프로퍼티가 get 접근자를 통해 m_Player_Move를 반환하는 것과 동일하게 동작

                @Move를 사용할 때마다 m_Wrapper.m_Player_Move가 참조되거나 작동
                @Move는 그저 m_Wrapper.m_Player_Move를 바로 접근하기 위한 일종의 단축키
            */
        ...
        /*  InputControlScheme이란 
            `InputControlScheme`은 Unity의 새로운 Input System에서 사용되는 개념으로, 다양한 입력 장치(키보드, 마우스, 게임패드 등)를 지원하는 게임에서 장치별로 입력 방식을 설정하기 위해 사용됩니다. 간단히 말해, 서로 다른 입력 장치를 위한 입력 매핑을 그룹화하여 하나의 스키마로 관리할 수 있는 구조입니다.

            주요 기능:

            - **입력 장치 그룹화**: 키보드, 마우스, 컨트롤러 등의 입력 장치를 위한 각각의 입력 매핑을 스키마로 정의할 수 있습니다.
            - **장치 자동 전환**: 게임 플레이 도중 사용자가 키보드에서 게임패드로 전환하는 등의 상황에서 입력 장치에 맞게 자동으로 스키마를 전환할 수 있습니다.
            - **플랫폼별 대응**: PC, 콘솔, 모바일 등 여러 플랫폼에 맞춘 입력 스키마를 설정하여, 각 플랫폼에 맞는 입력 방식을 자동으로 적용할 수 있습니다.

            즉, `InputControlScheme`은 서로 다른 입력 장치나 설정을 효과적으로 관리하고 전환할 수 있도록 돕는 역할을 합니다.

            게임 내에서 사용하려면 Input Action Asset에서 Control Schemes를 설정하고, 해당 스키마를 통해 플레이어의 입력을 처리할 수 있습니다.
        */
        ...
        /*  아래 두 메서드 쓴 이유 --> 인터페이스 준수 및 열거자 구현
            열거자 구현: GetEnumerator() 메서드를 통해 JUTPSInputControlls 클래스 내의 입력 액션들을 순차적으로 열거할 수 있습니다.
            IInputActionCollection 인터페이스는 IEnumerable<InputAction>을 상속받고 있습니다. 
            따라서 이를 구현함으로써 해당 인터페이스의 계약을 충족시킵니다.

            IEnumerable<InputAction> 및 비제네릭 IEnumerable 인터페이스를 구현하여, 다양한 방식으로 클래스를 사용할 수 있게 합니다.
        */
